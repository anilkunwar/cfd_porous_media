...
if viz_library == 'Plotly (Interactive)':
    ...
    # Helper function to create Plotly surface — ✅ FULLY CORRECTED
    def create_plotly_surface(tensor_matrix, title, colorbar_title, is_resistivity=False):
        ...
        # ✅ FIXED: Use 'title_side' instead of deprecated 'titleside'
        colorbar_config = None
        if show_colorbar:
            colorbar_config = dict(
                title=colorbar_title,
                title_side="right",  # ← CORRECT PARAMETER NAME (was 'titleside')
                thickness=20,
                len=0.75
            )
    ...
    if show_axes:
        ...
        if visualization_option == 'Both':
            axis_length = max(
                np.max(np.abs(X_perm)), np.max(np.abs(Y_perm)), np.max(np.abs(Z_perm)),
                np.max(np.abs(X_res)), np.max(np.abs(Y_res)), np.max(np.abs(Z_res))
            ) * 1.2
        else:
            axis_length = max(np.max(np.abs(X)), np.max(np.abs(Y)), np.max(np.abs(Z))) * 1.2
